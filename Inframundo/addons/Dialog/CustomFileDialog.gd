...
var default_icons = {
	...
	"font"			: preload("Graphics/font.png"),
	"audio"			: preload("Graphics/file_audio.png"),
}
...
func _process(delta: float) -> void:
	if process_file_stack.size() > 0:
		...
		for i in _to:
			...
			elif file.extension.to_lower() == "ttf":
				_texture = default_icons.font
			elif file.extension.to_lower() == "wav" or \
				file.extension.to_lower() == "ogg":
				_texture = default_icons.audio
			else:
